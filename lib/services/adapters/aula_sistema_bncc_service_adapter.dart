...
class AulaSistemaBnccServiceAdapter {
  Future<List<AulaSistemaBncc>> listarDeAulaEspecifica({required String aulaOfflineId}) async {
    
    Box<AulaSistemaBncc> _aulaSistemaBnccBox = Hive.box('aula_sistema_bncc_offline');
    
    print('AULA BNCC BOX GERAL TOT: ' + _aulaSistemaBnccBox.values.toString());
    
    _aulaSistemaBnccBox.values.forEach((element) {
      print(element.aula_id.toString() + '-----' + element.sistema_bncc_id.toString());
    });

    List<AulaSistemaBncc> aulaSistemaBnccSalvos = _aulaSistemaBnccBox.values
  ...
  Future<void> salvarVarios({required List<SistemaBncc> sistemaBncc, required String aulaOfflineId}) async {
      Box<AulaSistemaBncc> _aulaSistemaBnccBox = Hive.box('aula_sistema_bncc_offline');

    if (sistemaBncc.isNotEmpty) {
      // ignore: avoid_function_literals_in_foreach_calls
      sistemaBncc.forEach((sistema) {
        _aulaSistemaBnccBox.add(AulaSistemaBncc(
